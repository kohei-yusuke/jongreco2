  ...
  return (
    <div className="jr-card">
      <div className="jr-card-head">
        <h5 className="jr-card-title">📋 スコア履歴</h5>
        <span className="jr-chip jr-chip-muted">{scores.length} 半荘</span>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table className="jr-table">
          ...
          <tbody>
            {scores.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center text-muted py-4" style={{ color: 'var(--jr-ink-muted)' }}>
                  まだ記録がありません
                </td>
              </tr>
            )}
            {scores.map((score) => {
              ...
              return (
                <tr key={score.id}>
                  <td className="rd">{score.round}</td>
                  {SEATS.map((s) => (
                    <td key={s} className={`num ${scoreClass(result[s].final)}`}>
                  ...
                  <td>
                    <button
                      ...
                      style={{ border: 0, background: 'transparent', color: 'var(--jr-ink-muted)', fontSize: '1rem', cursor: 'pointer', minHeight: 36, minWidth: 36 }}
                    >🗑</button>
                  </td>
                </tr>
              );
            ...
            {chipEnabled && (
              <tr>
                <td className="rd">チップ</td>
                {SEATS.map((s) => (
                  <td key={s} className="num">
                    <div className="chip-input">
                      <button
                        type="button"
                        className={chipInputs[s].isPositive ? 'plus' : 'minus'}
          ...
          <tfoot>
            <tr>
              ...
              {SEATS.map((s) => (
                <td key={s} className={`num jr-total ${scoreClass(totals[s])}`}>{formatSigned(totals[s])}</td>
              ))}
              <td></td>
            </tr>
            <tr>
              ...
              {SEATS.map((s) => (
                <td key={s}>
                  <span className="rank-pill">{RANK_MEDAL[ranks[s] - 1]}<span className={ranks[s] === 1 ? 'rank-1' : ''}>{ranks[s]}位</span></span>
                </td>
              ))}
              <td></td>
            </tr>
